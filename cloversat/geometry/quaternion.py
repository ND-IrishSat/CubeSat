...
Purpose:  All quaternion operations used by estimation and control.
Inputs:   Quaternions ``[w, x, y, z]`` (scalar first), vectors in SI.
Outputs:  Quaternions, rotated vectors, rotation matrices.
Status:   [MODIFIED]
Port from: legacy/Utils/transformations.py (``quaternion_multiply``,
           ``delta_q``, ``normalize``, ``quaternion_rotation_matrix``,
           ``vector_to_quaternion``)

Convention: ``q`` rotates inertial -> body (body <- inertial), so
``v_body = rotate(q, v_inertial)``. Pure functions, no globals.

Related open questions: none.

Naming: the normalize function is ``unit()`` so the legacy ``normalize`` in
the reference block below does not shadow it.

Porting notes:
    - Legacy ``quaternion_rotation_matrix`` returns local -> global
      (body -> inertial); check direction against the convention above.
    - Legacy ``delta_q`` computes ``q_actual * conj(q_target)`` and flips
      sign; confirm order against ``control/attitude_pd_law.py``.
"""
...
def unit(q: Quat) -> Quat:
    """Normalize: return ``q / |q|``. Raises ``ValueError`` on a zero-norm input."""
    raise NotImplementedError


def rotate_vector(q: Quat, v: Vec3) -> Vec3:
    """Rotate ``v`` by ``q``: inertial-frame vector in, body-frame vector out."""
    raise NotImplementedError


def error_quaternion(q: Quat, q_target: Quat) -> Quat:
    """Rotation from ``q_target`` to ``q``, sign-flipped to the shortest path."""
    raise NotImplementedError


def shortest_path(q: Quat) -> Quat:
    """Return ``q`` or ``-q`` so that ``w >= 0``."""
    raise NotImplementedError


def to_rotation_matrix(q: Quat) -> npt.NDArray[np.float64]:
    """3x3 direction cosine matrix equivalent to ``q`` (body <- inertial)."""
    raise NotImplementedError


def from_rotation_matrix(dcm: npt.NDArray[np.float64]) -> Quat:
    """Unit quaternion from a 3x3 rotation matrix (body <- inertial)."""
    raise NotImplementedError


# ---- LEGACY START: legacy/Utils/transformations.py (reference only, not wired up) ----
'''
transformations.py
