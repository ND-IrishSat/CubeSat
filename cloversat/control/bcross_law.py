"""Detumble control law: B-cross, with B-dot fallback.

Purpose:  Magnetorquer dipole that removes body rate.
Inputs:   Body rate ``omega_body`` [rad/s], field ``B_body`` [T], gain k;
          for B-dot, a short history of ``B_body`` samples and their spacing [s].
Outputs:  Dipole ``m_body`` [A*m^2].
Status:   [MODIFIED]
Port from: legacy/Controllers/B_dot.py (``B_dot``)

Law:
    B-cross (gyro available):  m = -k (B x omega) / |B|^2
    B-dot   (no gyro):         m = -k dB/dt / |B|^2
    |B| == 0 -> return zero dipole (legacy printed and divided by zero).

Changes from legacy: returns a dipole, not voltages (voltage conversion is in
torquer_allocation); takes plain arrays instead of the ``Magnetorquer_Sat``
object; SI units in (legacy mixed uT/T).

Related open questions: OQ-4 (detumble threshold/time sets k).
...
def bdot_dipole(B_history_body: Sequence[Vec3], sample_dt: float, gain: float) -> Vec3:
    ...
    Args:
        ...
        sample_dt: Spacing between samples [s].
        gain: k.

    Returns:
        Dipole [A*m^2]; zero vector if |B| == 0 or history is too short.
    """
    raise NotImplementedError


# ---- LEGACY START: legacy/Controllers/B_dot.py (reference only, not wired up) ----
'''
B_dot.py
...
def B_dot(sat: Magnetorquer_Sat):
    ...
    # TODO(bug): prints on |B| = 0, then divides by zero anyway below. Should return zero dipole.
