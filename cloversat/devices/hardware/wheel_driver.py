...
Purpose:  Drives the reaction wheel motors (direction, brake, PWM) and reads
          hall-sensor speed; configures the control board over I2C.
Inputs:   ``WheelDriverParams`` (pins), ``WheelSpec`` list, GPIO/PWM/I2C buses.
Outputs:  ``WheelDriver`` (``WheelInterface``).
Status:   [MODIFIED] [BLOCKED: OQ-3]
Port from: legacy/HardwareInterface/reaction_wheels/motors.py,
           legacy/HardwareInterface/reaction_wheels/pcb_config.py

Changes from legacy:
    - N wheels from params, not one wheel on module-constant pins.
    - No blocking ``time.sleep`` in ``set_wheel`` (legacy ``slow_down`` blocks
      up to 2 s on direction change); direction reversal handled without
      stalling the control loop.
    - ``safe()`` replaces ``kill()``; no SIGINT handler or ``sys.exit`` in the driver.
    - Command meaning (PWM / torque / speed) depends on OQ-3.

Reference only: legacy/HardwareInterface/reaction_wheels/old_scripts/,
legacy/HardwareInterface/reaction_wheels/skeleton.ino, skeleton.py.
...
if TYPE_CHECKING:
    from cloversat.config.device_params import WheelDriverParams
    from cloversat.config.spacecraft_params import WheelSpec


class WheelDriver(WheelInterface):
    """Reaction wheels on the IrishSat control board."""

    def __init__(self, params: WheelDriverParams, specs: list[WheelSpec]) -> None:
        raise NotImplementedError

    def init(self) -> None:
        """Configure pins and board registers; all wheels stopped."""
        raise NotImplementedError

    def self_test(self) -> SelfTestResult:
        """Spin each wheel briefly and check the hall-sensor speed responds."""
        raise NotImplementedError
    ...
    def get_wheel_rpm(self, i: int) -> Sample[float]:
        """Return wheel ``i`` signed speed [rpm] from the hall sensor."""
        raise NotImplementedError

    def safe(self) -> None:
        """Zero PWM on all wheels. Callable anytime; must not raise."""
        raise NotImplementedError


# ---- LEGACY START: legacy/HardwareInterface/reaction_wheels/motors.py (reference only, not wired up) ----
'''
motors.py
...
import time
# [scaffold: disabled] import pigpio
import signal
import sys
...
# ---- LEGACY START: legacy/HardwareInterface/reaction_wheels/pcb_config.py (reference only, not wired up) ----
# NOTE: this block's main() shadows motors.py's main() above; both are reference only.
# [scaffold: disabled] import smbus2
